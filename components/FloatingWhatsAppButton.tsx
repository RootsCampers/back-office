...
export function FloatingWhatsAppButton({ className = '' }: FloatingWhatsAppButtonProps) {
  ...
  useEffect(() => {
    ...
    const dismissedThisSession = sessionStorage.getItem('whatsapp-tooltip-dismissed')
  ...
  const openWhatsApp = (messageType: 'general' | 'route' | 'vehicle' | 'admin' | 'guided' = 'general') => {
    const phoneNumber = "+12029884022"
    
    // Get translated messages
    const message = encodeURIComponent(t(`whatsapp_help.messages.${messageType}`))
  ...
  return (
    <>
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <div>
                  <div className="font-bold text-sm text-amber-900 flex items-center gap-1">
                    {t('whatsapp_help.options.guided_title')}
                    <span className="text-xs bg-amber-200 text-amber-800 px-2 py-0.5 rounded-full">Popular</span>
                  </div>
                  <div className="text-xs text-amber-700">{t('whatsapp_help.options.guided_description')}</div>
                </div>
              </button>
              ...
              <div className="relative py-2">
                ...
                <div className="relative flex justify-center">
                  <span className="px-2 text-xs text-gray-400 bg-white">Or ask about</span>
                </div>
              </div>
              ...
              >
                ...
                <div>
                  <div className="font-medium text-sm text-gray-800">{t('whatsapp_help.options.route_title')}</div>
                  <div className="text-xs text-gray-500">{t('whatsapp_help.options.route_description')}</div>
                </div>
              </button>
              ...
              >
                ...
                <div>
                  <div className="font-medium text-sm text-gray-800">{t('whatsapp_help.options.vehicle_title')}</div>
                  <div className="text-xs text-gray-500">{t('whatsapp_help.options.vehicle_description')}</div>
                </div>
              </button>
              ...
              >
                ...
                <div>
                  <div className="font-medium text-sm text-gray-800">{t('whatsapp_help.options.dates_title')}</div>
                  <div className="text-xs text-gray-500">{t('whatsapp_help.options.dates_description')}</div>
                </div>
              </button>
            </div>
          </motion.div>
      ...
      >
        ...
        >
          {isExpanded ? <X size={24} /> : <WhatsAppIcon size={26} className="text-white" />}
        </motion.div>
      </motion.button>
      
      {!isExpanded && (
        <div
          className="absolute -top-2 -left-2 bg-red-500 text-white text-xs rounded-full w-6 h-6 flex items-center justify-center animate-badge-pulse"
        >
          !
        </div>
      )}
